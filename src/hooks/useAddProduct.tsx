...
export default function useAddProduct():UseAddProductReturnType {
  ...
  const addProduct = useCallback(async (productVariantId:string, quantity:number) => {
    ...
    if (result.data) {
      addSale(result.data.addItemToOrder.subTotal, result.data.addItemToOrder.subTotalWithTax);
    }
